...
async function handleGenerateAmazon() {
    ...
    fetchPreview(sizeKey, previewZpl, btn, btnPrint, preview, 'Gerar ZPL e Visualizar');
}
...
async function handleGenerateManual() {
    ...
    fetchPreview(sizeKey, previewZpl, btn, btnPrint, preview, 'Gerar Etiqueta');
}

async function fetchPreview(sizeKey, zpl, btn, btnPrint, previewContainer, btnText) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            previewContainer.innerHTML = `<img src="${imageUrl}" class="w-full h-full object-contain bg-white rounded-lg">`;
